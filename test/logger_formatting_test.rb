...
class LoggerFormattingTest < Minitest::Test
  def setup
    ...
    @logger = Capissh::Logger.new(:output => @io, :level => 3)
  end

  def test_matching_with_style_and_color
    Capissh::Logger.formatters = [{ :match => /^err ::/, :color => :red, :style => :underscore, :level => 0 }]
    @logger.log(0, "err :: Error Occurred")
    assert @io.string.include? "\e[4;31merr :: Error Occurred\e[0m"
  end

  def test_style_without_color
    Capissh::Logger.formatters = [{ :match => /.*/, :style => :underscore, :level => 0 }]
  ...
  def test_formatter_log_levels
    Capissh::Logger.formatters = [{ :match => /.*/, :color => :blue, :level => 3 }]
    @logger.log(0, "test message")
