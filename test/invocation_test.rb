...
class InvocationTest < MiniTest::Unit::TestCase
  ...
  def test_default_io_proc_should_log_stdout_arguments_as_info
    ...
    ch[:options][:logger].expects(:info).with("data stuff", "out :: capissh")
  ...
  def test_default_io_proc_should_log_stderr_arguments_as_important
    ch = { :host => "capissh",
           :server => server("capissh"),
           :options => { :logger => mock("logger") } }
    ch[:options][:logger].expects(:important).with("data stuff", "err :: capissh")
  ...
  private
    ...
    def make_config
      Capissh::Configuration.new(:logger => stub_everything)
    end
    ...
    def prepare_command(command, sessions, options)
      ...
      Capissh::Command.expects(:process).with(&compare_args)
    end
end
