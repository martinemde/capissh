...
class CommandTest < MiniTest::Unit::TestCase
  def test_command_should_open_channels_on_all_sessions
    ...
    assert_equal "ls", Capissh::Command.new("ls", [s1, s2, s3]).commands_for.call(server('capissh')).first.first
  end

  def test_command_with_newlines_should_be_properly_escaped
    cmd = Capissh::Command.new("ls\necho", [mock_session])
    assert_equal "ls\\\necho", cmd.commands_for.call(server('capissh')).first.first
  end
  ...
  def test_command_with_pty_should_request_pty_and_register_success_callback
    ...
    Capissh::Command.new("ls", [session], :pty => true)
  end
  ...
  def test_command_with_env_key_should_have_environment_constructed_and_prepended
    ...
    Capissh::Command.new("ls", [session], :env => { "FOO" => "bar" })
  end
  ...
  def test_env_as_string_should_be_substituted_in_directly
    ...
    Capissh::Command.new("ls", [session], :env => "HOWDY=there")
  end
  ...
  def test_open_channel_should_set_options_key_on_channel
    channel = nil
    session = setup_for_extracting_channel_action { |ch| channel = ch }
    Capissh::Command.new("ls", [session], :data => "here we go")
  ...
  def test_successful_channel_with_shell_option_should_send_command_via_specified_shell
    ...
    Capissh::Command.new("ls", [session], :shell => "/bin/bash")
  end
  ...
  def test_successful_channel_with_shell_false_should_send_command_without_shell
    ...
    Capissh::Command.new("echo `hostname`", [session], :shell => false)
  end
  ...
  def test_on_data_should_invoke_callback_as_stdout
    ...
    called = false
    Capissh::Command.new("ls", [session]) do |ch, stream, data|
      called = true
  ...
  def test_stop_should_close_all_open_channels
    ...
    cmd = Capissh::Command.new("ls", sessions)
    cmd.stop!
  end
  ...
  def test_process_should_instantiate_command_and_process!
    ...
    Capissh::Command.expects(:new).with("ls -l", %w(a b c), {:foo => "bar"}).returns(cmd)
    Capissh::Command.process("ls -l", %w(a b c), :foo => "bar")
  end
  ...
  def test_input_stream_closed_when_eof_option_is_true
    ...
    session = setup_for_extracting_channel_action { |ch| channel = ch }
    channel.expects(:eof!)
    Capissh::Command.new("cat", [session], :data => "here we go", :eof => true)
