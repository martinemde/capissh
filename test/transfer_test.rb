...
class TransferTest < MiniTest::Unit::TestCase
  def test_class_process_should_delegate_to_instance_process
    Capissh::Transfer.expects(:new).with(:up, "from", "to", %w(a b c), {}).returns(mock('transfer', :process! => nil)).yields
    yielded = false
  ...
  def test_active_is_true_when_any_sftp_transfers_are_active
    ...
    transfer = Capissh::Transfer.new(:up, "from", "to", sessions, :via => :sftp)
  ...
  def test_active_is_true_when_any_scp_transfers_are_active
    ...
    transfer = Capissh::Transfer.new(:up, "from", "to", sessions, :via => :scp)
    assert_equal true, transfer.active?
  end
  ...
  [:up, :down].each do |direction|
    define_method("test_sftp_#{direction}load_from_file_to_file_should_normalize_from_and_to") do
      ...
      transfer = Capissh::Transfer.new(direction, "from-$CAPISSH:HOST$", "to-$CAPISSH:HOST$", sessions)
    end
    ...
    define_method("test_scp_#{direction}load_from_file_to_file_should_normalize_from_and_to") do
      ...
      transfer = Capissh::Transfer.new(direction, "from-$CAPISSH:HOST$", "to-$CAPISSH:HOST$", sessions, :via => :scp)
    end
  end
  ...
  def test_sftp_upload_from_IO_to_file_should_clone_the_IO_for_each_connection
    ...
    transfer = Capissh::Transfer.new(:up, StringIO.new("from here"), "/to/here-$CAPISSH:HOST$", sessions)
  end
  ...
  def test_scp_upload_from_IO_to_file_should_clone_the_IO_for_each_connection
    ...
    transfer = Capissh::Transfer.new(:up, StringIO.new("from here"), "/to/here-$CAPISSH:HOST$", sessions, :via => :scp)
  end
  ...
  def test_uploading_a_non_existing_file_should_raise_an_understandable_error
    s = session('app1')
    error = Capissh::Processable::SessionAssociation.on(ArgumentError.new('expected a file to upload'), s)
    transfer = Capissh::Transfer.new(:up, "from", "to", [], :via => :scp)
    transfer.expects(:process_iteration).raises(error)
    assert_raises(ArgumentError, 'expected a file to upload') { transfer.process! }
  end
